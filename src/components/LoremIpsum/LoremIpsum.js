...
class LoremIpsum extends Component {
    ...
    handleChange = (event) => {
        this.setState({ amount: parseInt(Math.abs(event.target.value)) });
    };
    ...
    handleSubmit = (event) => {
        ...
        if (isNaN(amount) || amount < 1 || amount > 9) {
            this.setState({ result: text[random] });
        } else {
        ...
        return (
            <section className="lorem-section-center">
                ...
                <form className="lorem-form" onSubmit={this.handleSubmit}>
                    ...
                    <button type="submit" className="btn lorem-btn">generate</button>
                </form>
